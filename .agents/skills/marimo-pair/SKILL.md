...
### Educational Notebook Pairing

When a marimo notebook is meant to teach a subject, also apply the local
`teaching-notebook-builder` skill if it is available. The notebook should teach
the domain, not merely run code: include a plain-English map, define key terms
before heavy use, explain controls and outputs, make charts interpretable, and
add student exercises or next experiments. Keep the explanations
high-school-friendly by default while preserving real rigor.

### Cloud/Local Source-of-Truth Contract

When pairing on a cloud marimo notebook, first determine the project source of
truth. On this machine, marimo projects usually live under
`/Volumes/CORSAIR/Marimo`, and a project's `AGENTS.md`, `README.md`,
`PLAN.md`, or `metadata.yaml` may designate the local notebook as the durable
source of truth while MoLab is only the execution and sharing surface.

If the live cloud runtime is the active source of truth for the task, make
notebook edits with `cm` or the pairing API, verify them in the running kernel,
and pull/export the saved cloud notebook back to the matching local folder.

If the local project is the durable source of truth, edit local `notebook.py`
only when no live cloud runtime is meant to receive incremental edits, verify it
locally, then push the local notebook into the live cloud runtime through `cm`
or the pairing API for execution. Do not expect direct local file edits to be
seen by an already-open cloud runtime until you explicitly push them.

Before calling a cloud notebook task complete after edits:

1. Verify the edited cloud cells ran or rendered as appropriate.
2. Confirm the saved cloud artifact contains the latest intended changes.
3. Pull/export the saved cloud notebook into the matching local project folder.
4. Compare cloud and local hashes or equivalent evidence. If MoLab normalizes
   script metadata, compare cell bodies and cell configs instead of raw file
   hashes.
5. Scan the saved notebook/local project for accidentally stored secrets.

If any save, pull-back, push, or equivalence check cannot be verified, report
that as an explicit remaining gate. Do not describe the notebook as saved,
synced, or done until the active execution surface and the durable source of
truth are confirmed to match for the chosen comparison method.
